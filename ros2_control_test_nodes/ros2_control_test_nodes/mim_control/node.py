...
# mim_control imports
from ros2_control_test_nodes.mim_control.mim_control.demo_robot_impedance import Demo as ImpedanceDemo
from ros2_control_test_nodes.mim_control.mim_control.demo_robot_com_ctrl import Demo as CentroidalDemo

# reactive planner import
from ros2_control_test_nodes.mim_control.reactive_planners.demo_reactive_planners_solo12_step_adjustment_walk import Demo as ReactivePlannerDemo
...
class RobotImpedanceController(Node):
    ...
    def __init__(self):
        ...
        # Service to start the mim_control control
        ...
        # Service to make a plot of the footstep positions
    ...
    def timer_callback(self):
        ...
        if self.curr_state == self.States.WALK:
            ...
            joint_config_with_base = np.concatenate((pose_fp, self.joint_config))
            joint_vel_with_base = np.concatenate((twist_fp, self.joint_velocity))
            tau = self.reactive_planner_demo.compute_torques(joint_config_with_base, joint_vel_with_base, self.control_time, "forward")
            msg.data = tau.tolist()
            self.publisher_.publish(msg)
